...
import { BookOpen, Code2, LayoutDashboard, Terminal, Eye } from "lucide-react";
            ...
            <div key={topic.id} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 md:p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {topic.tags.map((item, idx) => (
                  <div key={idx} className="bg-gray-50 rounded-xl p-5 border border-gray-100 hover:border-red-200 transition-colors group">
                    <div className="flex items-start justify-between mb-3">
                      <code className="px-2 py-1 bg-[#FF2D20]/10 text-[#FF2D20] rounded text-lg font-bold font-mono">
                        {item.tag}
                      </code>
                    </div>
                    <p className="text-sm text-gray-600 mb-4 h-16 overflow-y-auto custom-scrollbar">
                      {item.description}
                    </p>

                    {item.attributes.length > 0 && (
                      <div className="mb-4">
                        <h4 className="text-xs uppercase tracking-wider font-semibold text-gray-500 mb-2">Atribut Global</h4>
